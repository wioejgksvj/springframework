...
public class Ch03Controller {
	...
	public String receiveParamData(
		...
		//JSP로 데이터 전달
		model.addAttribute("param1", param1); // modelAndView 방식보다 이 방식을 많이 사용한다. / addAttribute로 추가
	...
	@PostMapping("/receivePostMethodForm") // ✔️기본속성 이름은 value / ("/") 소괄호랑 " 사이에 value 생략되어있다.
	public String receivePostMethodForm (
		String param1, 
		int param2, 
		double param3, 
		boolean param4, 
		@DateTimeFormat(pattern="yyyy-MM-dd") Date param5, // 데이트 어노테이션 사용시 두자리 월은 라지M / ✔️기본속성값으로 문자열을 안받을땐 다른속성 값을 (와"사이에 줘야한다.
		Model model) {
		...
		public String defaultValue (
				String param1, 
				@RequestParam(defaultValue="0") int param2, // 기본으로 null값이 들어가므로 requestParam 해줘야 한다.
				@RequestParam(defaultValue="0.0") double param3,
				@RequestParam(defaultValue="false") boolean param4,  // null을 true false 할 수 없으므로 default값으로 false를 준다.
				String param5,
				Model model) {
		...
		public String otherArgName (
				...
				@RequestParam(value="param2", defaultValue="0") int arg2, // 안에 값이 2개 이상일때 value 생략이 불가로 직접 적어야 한다.
		...
		public String commandObject (Ch03Dto dto, Model model) {
			...
			log.info("param4: " + dto.isParam4()); // boolean 타입은 isParam
